...
> **Self-hosted OIDC/OAuth2 provider — the identity backbone for every MADFAM service.**
...
| `janua-app` | app.janua.dev | — |
| `janua-docs` | docs.janua.dev | — |
| `janua-web` | janua.dev | — |

**Kubernetes namespace**: `janua`
...
### Day-to-day for janua-api

The commands below default to `janua-api` — the primary service name for
...
enclii ps --wide
enclii ps janua-api --env production

# Logs (tail, filter, history)
enclii logs janua-api -f                          # live tail
enclii logs janua-api --since 1h --level error    # last hour, errors only
enclii logs janua-api --env staging -f
...
# Rollback
enclii rollback janua-api                         # previous release
enclii rollback janua-api --to-revision 5

# Releases + history
enclii releases janua-api                          # list builds
enclii releases janua-api --latest --output json

# Secrets (routed through Lockbox → Vault → ESO → K8s)
enclii secrets list janua-api
enclii secrets set MY_KEY=value --service janua-api --secret
enclii secrets rm MY_KEY --service janua-api

# Domains, tunnel routes, DNS
enclii domains list janua-api
enclii domains add janua-api my.example.com       # auto-provisions tunnel route + DNS
...
# Routing (ingress + TLS)
enclii junctions list janua-api
