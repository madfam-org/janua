# Janua Documentation

Complete documentation for the Janua self-hosted authentication platform.

## 📚 Documentation Structure

### Getting Started
- **[Quick Start Guide](guides/QUICK_START.md)** - Get Janua running in 5 minutes
- **[Installation](developers/getting-started/README.md)** - Complete installation instructions
- **[Development Guide](development/DEVELOPMENT.md)** - Local development setup

### Core Documentation
- **[API Reference](api/API_REFERENCE.md)** - Complete API documentation
- **[Architecture](architecture/ARCHITECTURE.md)** - System architecture and design
- **[Deployment](deployment/DEPLOYMENT.md)** - Production deployment guide
- **[Security](security/SECURITY.md)** - Security features and best practices

### User Guides
- **[React SDK](guides/react-sdk-complete-guide.md)** - React integration guide
- **[Vue SDK](guides/vue-sdk-complete-guide.md)** - Vue.js integration guide
- **[Flutter SDK](guides/flutter-sdk-complete-guide.md)** - Flutter mobile integration
- **[SSO Integration](guides/SSO_INTEGRATION_GUIDE.md)** - Single Sign-On setup
- **[MFA Guide](guides/mfa-2fa-implementation-guide.md)** - Multi-factor authentication
- **[Compliance Features](guides/COMPLIANCE_FEATURES_GUIDE.md)** - GDPR, SOC2, compliance

### Enterprise Features
- **[Enterprise Overview](enterprise/ENTERPRISE_FEATURES.md)** - Enterprise capabilities
- **[SSO Guide](enterprise/sso-integration-guide.md)** - Enterprise SSO setup
- **[RBAC Setup](enterprise/enterprise-rbac-setup-guide.md)** - Role-based access control
- **[SCIM Provisioning](guides/SCIM_PROVISIONING_GUIDE.md)** - User provisioning
- **[Audit & Compliance](enterprise/audit-compliance-guide.md)** - Enterprise compliance

### Migration & Integration
- **[Cloud to Self-Hosted](migration/cloud-to-self-hosted.md)** - Migration guide
- **[Auth0 Migration Tool](../scripts/migration/README.md)** - Auth0 to Janua migration
- **[Integrations](enterprise/integrations/README.md)** - Third-party integrations

### Reference
- **[API Specification](reference/API_SPECIFICATION.md)** - OpenAPI specification
- **[Troubleshooting](troubleshooting/common-issues.md)** - Common issues and solutions
- **[Changelog](CHANGELOG.md)** - Version history
- **[Roadmap](ROADMAP.md)** - Future plans

### Internal Documentation
- **[Reports](internal/reports/)** - Analysis and assessment reports
- **[Operations](internal/operations/)** - Operational procedures
- **[Testing](internal/testing/)** - Test documentation
- **[Claude Sessions](internal/claude-sessions/)** - Development session notes

### Archives
- **[Historical](historical/)** - Archived/deprecated documentation
- **[2025-11 Archive](archive/2025-11/)** - November 2025 archived docs
- **[Rebranding Docs](.archive/rebranding-2025-11/)** - Plinto → Janua rebranding documentation

## 🔍 Quick Links

### For Developers
- [API Documentation](api/API_DOCUMENTATION.md)
- [SDKs Overview](../packages/)
- [GraphQL API](GRAPHQL_API_REFERENCE.md)
- [Testing Guide](E2E_TESTING_GUIDE.md)

### For DevOps
- [Deployment Guide](deployment/DEPLOYMENT.md)
- [Monitoring Setup](deployment/MONITORING_SETUP.md)
- [Production Readiness](deployment/PRODUCTION_READINESS_REPORT.md)
- [Railway Deployment](internal/operations/RAILWAY_DEPLOYMENT.md)

### For Product Teams
- [User Journeys](user-journeys/)
- [Beta Launch Checklist](BETA_LAUNCH_CHECKLIST.md)
- [Beta Onboarding](BETA_ONBOARDING_GUIDE.md)
- [Marketing Design](guides/MARKETING_DESIGN.md)

### For Business
- [Business Development](business/BIZ_DEV.md)
- [Comparison to Alternatives](../README.md#comparison-to-alternatives)
- [Pricing Strategy](../README.md#is-this-production-ready)

## LLM-Friendly Documentation

Janua follows the [llmstxt.org](https://llmstxt.org) specification for AI-readable documentation:
- **[llms.txt](../llms.txt)** - Concise documentation index
- **[llms-full.txt](../llms-full.txt)** - Complete technical reference
- **[Online](https://janua.dev/llms.txt)** - Web-accessible version

## 📖 Documentation Standards

This documentation follows these principles:

1. **Accuracy First** - All examples are tested and functional
2. **Up-to-Date** - Documentation is versioned and maintained
3. **User-Focused** - Written for the intended audience (developers, DevOps, business)
4. **Complete** - All features documented with examples
5. **Searchable** - Clear structure with cross-references

## 🤝 Contributing

Found an issue or want to improve documentation?

1. Read [CONTRIBUTING.md](../CONTRIBUTING.md)
2. Check [Documentation Guidelines](CONTENT_GUIDELINES.md)
3. Submit a pull request

## 📬 Support

- **GitHub Issues**: [Report bugs or request features](https://github.com/madfam-org/janua/issues)
- **Documentation Issues**: Tag with `documentation` label
- **Security Issues**: security@janua.dev (private disclosure)

---

**Last Updated**: November 22, 2025  
**Documentation Version**: 1.0.0 (Post-Rebranding)
