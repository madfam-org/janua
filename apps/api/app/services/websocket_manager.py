...
class ConnectionManager:
    ...
    async def connect(
        self, websocket: WebSocket, user_id: Optional[str] = None, auth_token: Optional[str] = None
    ) -> str:
        """
    ...
    async def authenticate(self, connection_id: str, auth_token: str, db_session) -> bool:
        """
        ...
        if not user:
            await self.send_to_connection(
                connection_id,
                {
                    ...
                    "data": {"status": "failed", "error": "Invalid authentication token"},
                },
            )
            return False
        ...
        await self.send_to_connection(
            ...
            {
                "type": EventType.AUTHENTICATION,
                "data": {"status": "success", "user_id": str(user.id), "email": user.email},
            },
        )

        return True

    async def subscribe(self, connection_id: str, subscription_type: str, target_id: str) -> bool:
        """
        ...
        await self.send_to_connection(
            connection_id,
            {
                ...
                "data": {"status": "subscribed", "type": subscription_type, "target": target_id},
            },
        )

        return True

    async def unsubscribe(self, connection_id: str, subscription_type: str, target_id: str) -> bool:
        """
        ...
        await self.send_to_connection(
            connection_id,
            {
                ...
                "data": {"status": "unsubscribed", "type": subscription_type, "target": target_id},
            },
        )
    ...
    async def broadcast_to_organization(
        self, organization_id: str, message: dict, exclude_connection: Optional[str] = None
    ):
        """
    ...
    async def broadcast_to_topic(
        self, topic: str, message: dict, exclude_connection: Optional[str] = None
    ):
        """
    ...
    async def broadcast_to_all(self, message: dict, authenticated_only: bool = True):
        """
    ...
    async def handle_message(self, connection_id: str, message: dict, db_session):
        """
        ...
        if message_type == EventType.AUTHENTICATION:
            ...
            await self.send_to_connection(
                connection_id,
                {"type": EventType.PONG, "data": {"timestamp": datetime.utcnow().isoformat()}},
            )
    ...
    async def _handle_custom_message(self, connection_id: str, data: dict, db_session):
        """
        ...
        if not connection or not connection["authenticated"]:
            await self.send_to_connection(
                connection_id,
                {"type": EventType.ERROR, "data": {"error": "Authentication required"}},
            )
            return
    ...
    def get_stats(self) -> dict:
        ...
        authenticated_count = sum(
            1 for c in self.active_connections.values() if c.get("authenticated", False)
        )
        ...
        return {
            ...
            "total_subscriptions": sum(
                len(c.get("subscriptions", set())) for c in self.active_connections.values()
            ),
        }
