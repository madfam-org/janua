...
class AuditService:
    ...
    async def log_data_access(
        ...
        metadata = {"data_fields": data_fields, "lawful_basis": lawful_basis, "purpose": purpose}
    ...
    async def generate_compliance_report(
        ...
        try:
            ...
            return {
                ...
                "categories": {category: len(events) for category, events in categories.items()},
    ...
    def _calculate_risk_level(
        self, action: str, success: bool, error_message: Optional[str]
    ) -> str:
    ...
    def _assess_compliance_status(self, logs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        failed_high_risk = [
            log for log in logs if not log["success"] and log["risk_level"] in ["high", "critical"]
        ]
        if failed_high_risk:
        ...
        return {"status": status, "issues": issues, "total_violations": len(issues)}
