...
class AlertRuleCreate(BaseModel):
    ...
    comparison_operator: str = Field(..., regex="^(>|<|>=|<=|==|!=)$", description="Comparison operator")
...
async def get_notification_channels(
    ...
    try:
        ...
        for channel in alert_manager.notification_channels.values():
            ...
            safe_config = {k: "[MASKED]" if k.lower() in ["password", "token", "secret", "key", "webhook_url"]
...
async def create_notification_channel(
    ...
    try:
        ...
        if alert_manager.redis_client:
            ...
            await alert_manager.redis_client.hset(f"alert:channel:{channel_id}", mapping=channel_data_dict)
