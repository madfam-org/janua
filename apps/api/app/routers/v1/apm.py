    ...
    try:
        ...
        for trace_id in trace_ids:
            ...
            for span_id in span_ids:
                span_data = await apm_collector.redis_client.hgetall(
                    f"apm:trace:{trace_id}:{span_id}"
                )
                if span_data:
        ...
        return {
            ...
            "filters": {"operation": operation, "status": status, "limit": limit},
        }
...
async def get_trace_details(
    trace_id: str, current_user: User = Depends(get_current_admin_user)
) -> Dict[str, Any]:
    ...
    try:
        ...
        if operation:
            ...
            profile_data = await apm_collector.redis_client.zrevrange(
                f"apm:performance:{operation}", 0, limit - 1, withscores=True
            )
                ...
                profiles.append(
                    {
                        ...
                        "cpu_usage": float(profile_data.get("cpu_usage", 0))
                        if profile_data.get("cpu_usage")
                        else None,
                        "memory_usage": int(profile_data.get("memory_usage", 0))
                        ...
                        "custom_metrics": eval(profile_data.get("custom_metrics", "{}"))
                        if profile_data.get("custom_metrics")
                        else {},
                    }
                )
        ...
        return {
            ...
            "filters": {"operation": operation, "min_duration": min_duration, "limit": limit},
        }
...
async def get_system_metrics(
    ...
    try:
        ...
        metrics_data = await apm_collector.redis_client.zrangebyscore(
            "apm:system_metrics", start_time, end_time, withscores=True
        )
        ...
        for metric_key, timestamp in metrics_data:
            ...
            if metric_type == "cpu":
                ...
                memory_metrics.append(
                    {"timestamp": datetime.fromtimestamp(float(ts)).isoformat(), "value": timestamp}
                )
...
async def cleanup_old_data(
    ...
    try:
        ...
        # Clean up old traces
        old_traces = await apm_collector.redis_client.zrangebyscore("apm:traces", 0, cutoff_time)
