...
class OIDCProviderSetupRequest(BaseModel):
    ...
    scopes: Optional[list] = Field(None, description="OAuth 2.0 scopes (default: openid, profile, email)")
...
async def discover_from_url(
    request: OIDCDiscoveryFromURLRequest,
    current_user: User = Depends(get_current_user)
):
    """
...
async def setup_oidc_provider(
    request: OIDCProviderSetupRequest,
    current_user: User = Depends(get_current_user)
):
    """
