...
class OIDCProtocol(SSOProtocol):
        ...
        **kwargs,
        ...
        if self.cache_service:
            await self.cache_service.set(
                f"oidc_state:{state}", request_state, ttl=600  # 10 minutes
            )
    ...
    async def handle_callback(self, callback_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        id_token_payload = await self._validate_id_token(
            tokens["id_token"], config, request_state["nonce"]
        )
        ...
        if config.get("userinfo_endpoint") and tokens.get("access_token"):
            user_info = await self._fetch_user_info(
                tokens["access_token"], config["userinfo_endpoint"]
            )
        ...
        user_data = self.attribute_mapper.map_oidc_claims(
            user_claims, config.get("attribute_mapping", {})
        )
    ...
    async def refresh_tokens(self, refresh_token: str, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def revoke_token(
        self, token: str, config: Dict[str, Any], token_type: str = "access_token"
    ) -> bool:
    ...
    async def _validate_id_token(
        self, id_token: str, config: Dict[str, Any], expected_nonce: str
    ) -> Dict[str, Any]:
        """Validate and decode ID token"""
    ...
    async def _get_signing_key(self, kid: Optional[str], config: Dict[str, Any]) -> str:
    ...
    async def _fetch_user_info(self, access_token: str, userinfo_endpoint: str) -> Dict[str, Any]:
