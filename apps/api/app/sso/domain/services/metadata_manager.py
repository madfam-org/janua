...
class MetadataManager:
    ...
    def generate_sp_metadata(
        ...
        root = ET.Element(
            "md:EntityDescriptor",
            {
                ...
                "entityID": entity_id,
                "validUntil": (datetime.utcnow() + timedelta(hours=validity_hours)).isoformat()
                + "Z",
            },
        )
        ...
        if contact_email:
            contact = ET.SubElement(root, "md:ContactPerson", {"contactType": "technical"})
    ...
    def validate_metadata(self, metadata_xml: str, metadata_type: str = "idp") -> Dict[str, Any]:
        """
                    ...
                    for cert in parsed["certificates"]:
                        if not cert["validation"]["valid"]:
                            result["errors"].append(
                                f"Invalid certificate: {cert['validation'].get('error')}"
                            )
                            result["valid"] = False
    ...
    def _add_key_descriptor(
        self, parent: ET.Element, certificate_pem: str, use: str = "signing"
    ) -> None:
