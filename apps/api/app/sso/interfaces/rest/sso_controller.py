...
class SSOController:
    ...
    async def initiate_sso(self, request: SSOInitiateRequest, db: AsyncSession = Depends(get_db)):
    ...
    async def handle_saml_callback(self, request: Request, db: AsyncSession = Depends(get_db)):
        ...
        try:
            ...
            result = await orchestrator.handle_authentication_callback(
                protocol="saml2", callback_data=callback_data
            )
    ...
    async def handle_oidc_callback(self, request: Request, db: AsyncSession = Depends(get_db)):
        ...
        try:
            ...
            result = await orchestrator.handle_authentication_callback(
                protocol="oidc", callback_data=callback_data
            )
    ...
    async def configure_sso(
        ...
        try:
            ...
            is_valid = await orchestrator.validate_protocol_configuration(
                protocol=request.protocol, config=request.config
            )
    ...
    def _has_admin_access(self, user, organization_id: str) -> bool:
        ...
        return user.organization_id == organization_id and user.role in ["admin", "owner"]
