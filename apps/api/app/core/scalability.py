...
class ResourceMonitor:
    ...
    async def _check_scaling_conditions(self, metrics: ScalabilityMetrics):
        ...
        if (
            ...
            or metrics.memory_usage_percent > 80
            or metrics.average_response_time_ms > 200
        ):
...
class HorizontalScalingManager:
    ...
    async def register_instance(self):
        ...
        logger.info(
            f"Instance registration data: {json.dumps(registration_data, indent=2, default=str)}"
        )
    ...
    async def health_check_extended(self) -> Dict[str, Any]:
        ...
        basic_health = {
            ...
            "uptime_seconds": (
                datetime.now() - self.instance_metadata["started_at"]
            ).total_seconds(),
        }
        ...
        try:
            ...
            healthy = (
                redis_healthy
                and cpu_percent < 90
                and memory_percent < 95  # Not critically overloaded  # Not out of memory
            )
            ...
            basic_health.update({"status": "degraded", "error": str(e)})
...
class MultiRegionCoordination:
    ...
    def _get_failover_priority(self) -> int:
        ...
        priorities = {"us-east-1": 1, "eu-west-1": 2, "us-west-2": 3}
...
class GracefulShutdownHandler:
    ...
    async def initiate_shutdown(self):
        ...
        while self.active_requests > 0 and time.time() - shutdown_start < self.shutdown_timeout:
