...
class AlertOrchestrator:
    ...
    async def resolve_alert(
        self, alert_id: str, resolved_by: Optional[str] = None, reason: Optional[str] = None
    ) -> bool:
        """Resolve an alert"""
        try:
            alert = await self._get_alert_with_validation(alert_id)
            ...
            await self._record_lifecycle_event(
                ...
                alert_id,
                alert.rule_id,
                {"resolved_by": resolved_by or "system", "resolution_reason": reason},
            )
    ...
    async def escalate_alert(self, alert_id: str, escalation_reason: str) -> bool:
        ...
        try:
            ...
            if rule:
                await self._notification_dispatcher.send_alert_notifications(
                    alert, rule, notification_type="escalation"
                )
    ...
    async def get_alert_metrics(self, hours: int = 24) -> Dict[str, any]:
        ...
        try:
            ...
            recent_events = [event for event in self._lifecycle_events if event.timestamp >= cutoff]
            ...
            for alert in active_alerts:
                severity_distribution[alert.severity.value] = (
                    severity_distribution.get(alert.severity.value, 0) + 1
                )
                status_distribution[alert.status.value] = (
                    status_distribution.get(alert.status.value, 0) + 1
                )
            ...
            if acknowledged_alerts:
                ack_times = [
                    a.time_to_acknowledge_minutes()
                    for a in acknowledged_alerts
                    if a.time_to_acknowledge_minutes()
                ]
    ...
    async def _refresh_caches(self) -> None:
        ...
        await asyncio.gather(self._refresh_active_alerts_cache(), self._refresh_rules_cache())
    ...
    async def _record_lifecycle_event(
        self, event_type: str, alert_id: str, rule_id: str, metadata: Dict[str, any]
    ) -> None:
