...
class RuleEvaluationHistory:
    ...
    def get_breach_rate(self, rule_id: str, window_minutes: int = 60) -> float:
        ...
        recent_results = [
            result
            for result, timestamp in zip(self._history[rule_id], self._timestamps[rule_id])
            if timestamp >= cutoff
        ]
    ...
    async def evaluate_rule(self, rule: AlertRule) -> EvaluationResult:
        ...
        try:
            ...
            current_value = await self._metrics_provider.get_metric_value(
                rule.condition.metric_name, rule.condition.evaluation_window_seconds
            )
...
class AlertEvaluationOrchestrator:
    ...
    ) -> Tuple[List[Alert], List[EvaluationResult]]:
    ...
    async def evaluate_for_auto_resolution(
        self, active_alerts: List[Alert], rules: Dict[str, AlertRule]
    ) -> List[Alert]:
        ...
        for alert in active_alerts:
            ...
            try:
                ...
                logger.error(
                    f"Failed to evaluate alert {alert.alert_id} for auto-resolution", error=str(e)
                )
