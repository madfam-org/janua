...
async def change_password(
    ...
    try:
        ...
        # This forces re-login on all devices and blacklists all tokens
        revoked_count = await AuthService.invalidate_user_sessions(db, current_user.id)
        logger.info(
...
async def revoke_all_sessions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to revoke sessions"
        )
...
async def delete_account(
    ...
    try:
        ...
        await db.execute(
            ...
            .where(Session.user_id == current_user.id)
            .where(Session.is_active == True)
            .values(is_active=False, revoked_at=datetime.utcnow(), revoked_reason="account_deleted")
        )
