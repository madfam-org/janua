...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if self.redis_client:
            ...
            remaining = await self._get_remaining_requests(
                f"rate_limit:ip:{client_ip}", self.default_limit
            )
    ...
    async def _check_rate_limit(self, key: str, limit: int, window: int, identifier: str) -> None:
        ...
        try:
            ...
            if request_count >= limit:
                ...
                if oldest_request:
                    retry_after = int(oldest_request[0][1] + window - current_time + 1)
                else:
                    retry_after = window
    ...
    async def _get_remaining_requests(self, key: str, limit: int) -> int:
    ...
    async def _track_violation(
        self, client_ip: str, tenant_id: Optional[str], endpoint: str
    ) -> None:
    ...
    async def _ban_client(self, client_ip: str, duration: int = 3600) -> None:
            ...
            try:
                ...
                async with get_db_session() as db:
                    result = await db.execute(
                        select(Organization.billing_plan).where(Organization.id == tenant_id)
                    )
                    org_plan = result.scalar_one_or_none()

                    if org_plan:
                        # Cache the plan for future requests (1 hour TTL)
                        await self.redis_client.setex(plan_key, 3600, org_plan)  # 1 hour cache
