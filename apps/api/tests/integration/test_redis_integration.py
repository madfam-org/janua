...
class TestRedisIntegration:
    ...
    async def test_redis_dependency_injection(self):
...
class TestSessionStoreIntegration:
    ...
    async def test_multi_user_session_management(self):
        ...
        all_session_keys = [f"session:{sid}" for sid in user1_sessions + user2_sessions]
...
class TestRedisPerformanceIntegration:
    ...
    async def test_concurrent_rate_limit_checks(self):
        ...
        tasks = [self.rate_limiter.check_rate_limit(key, 10, 60) for key in keys]
    ...
    async def test_concurrent_session_operations(self):
        ...
        session_data_list = [{"user_id": f"user_{i}", "data": f"data_{i}"} for i in range(5)]
        ...
        get_tasks = [self.session_store.get_session(sid) for sid in session_ids]
