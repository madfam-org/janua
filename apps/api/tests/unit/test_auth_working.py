...
class TestAuthenticationWorking:
    ...
    async def test_signin_success(self, mock_get_db, mock_auth_service):
        ...
        mock_service.authenticate = AsyncMock(
            return_value={"id": "user_123", "email": self.test_email, "full_name": "Test User"}
        )
        mock_auth_service.return_value = mock_service
        ...
        with patch("app.services.jwt_service.JWTService") as mock_jwt:
            ...
            mock_jwt_instance.create_token_pair = MagicMock(
                return_value={
    ...
    async def test_duplicate_email_prevention(self, mock_get_db, mock_auth_service):
        ...
        mock_service.get_user_by_email = AsyncMock(
            return_value={"id": "existing_user", "email": self.test_email}
        )
        mock_auth_service.return_value = mock_service
