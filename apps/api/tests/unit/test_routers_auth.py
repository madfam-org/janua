        ...
        with patch("app.services.auth_service.AuthService") as mock_auth_service, patch(
            ...
            auth_routes = [
                ...
                if any(
                    ...
                    for keyword in ["login", "register", "token", "logout", "refresh"]
                )
            ]
    ...
    try:
        with patch("app.services.auth_service.AuthService") as mock_auth_service, patch(
            ...
            router_funcs = [
                ...
                if callable(getattr(auth, attr)) and not attr.startswith("_")
            ]
            auth_funcs = [
                func
...
def test_auth_dependencies(mock_env):
    ...
    try:
        ...
        with patch("app.services.auth_service.AuthService") as mock_auth_service, patch(
            "app.core.database_manager.DatabaseManager"
        ) as mock_db_manager, patch("app.models.User") as mock_user, patch(
            "app.models.Session"
        ) as mock_session:
            mock_auth_service.return_value = MagicMock()
            mock_db_manager.return_value = MagicMock()
