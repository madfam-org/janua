...
class TestHealthValidator:
    ...
    def validate_configuration(self):
        ...
        config_files = ["pytest.ini", "pyproject.toml", "tests/conftest.py"]
    ...
    def run_test_suites(self):
        ...
        test_commands = [
            (
                "Core Tests",
                ["python", "-m", "pytest", "tests/test_100_coverage.py", "-q", "--tb=no"],
            ),
            (
                "Unit Tests",
                ["python", "-m", "pytest", "tests/unit", "-q", "--tb=no", "--maxfail=5"],
            ),
            (
                "Integration Tests",
                ["python", "-m", "pytest", "tests/integration", "-q", "--tb=no", "--maxfail=5"],
            ),
        ]
    ...
    def generate_report(self) -> Dict:
        ...
        for suite, results in self.results["test_suites"].items():
            if isinstance(results, dict) and "passed" in results:
                print(
                    f"  {suite}: {results['status']} (✅ {results['passed']} ❌ {results['failed']} ⏩ {results['skipped']})"
                )
            else:
        ...
        passed_tests = sum(
            r.get("passed", 0) for r in self.results["test_suites"].values() if isinstance(r, dict)
        )
