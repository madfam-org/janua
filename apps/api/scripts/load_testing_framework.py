...
class LoadTestRunner:
    ...
    async def setup_session(self):
        ...
        logger.info(
            f"HTTP session initialized with {self.config.concurrent_users} concurrent connections"
        )
        ...
        for i in range(count):
            ...
            try:
                ...
                async with self.session.post(
                    f"{self.config.base_url}/beta/signup", json=signup_data
                ) as response:
    ...
    async def authenticate_user(self, user: Dict[str, str]) -> Optional[str]:
        ...
        try:
            ...
            async with self.session.post(
                f"{self.config.base_url}/beta/signin", json=signin_data
            ) as response:
    ...
    async def make_request(
        ...
        try:
            ...
            async with self.session.request(
                method=method, url=full_url, headers=request_headers, json=json_data
            ) as response:
    ...
    async def health_check_workload(self, user_id: str) -> List[TestResult]:
        ...
        endpoints = [("GET", "/health"), ("GET", "/ready"), ("GET", "/"), ("GET", "/api/status")]
    ...
    async def authentication_workload(self, user_id: str) -> List[TestResult]:
        ...
        signin_result = await self.make_request(
            "POST", "/beta/signin", json_data={"email": user["email"], "password": user["password"]}
        )
            ...
            if token:
                ...
                for _ in range(3):
                    session_result = await self.make_request(
                        "GET", "/api/v1/sessions/current", headers=auth_headers
                    )
    ...
    async def ramp_up_users(self) -> List[asyncio.Task]:
        ...
        logger.info(
            f"Ramping up {self.config.concurrent_users} users over {self.config.ramp_up_seconds} seconds"
        )
    ...
    def analyze_results(self) -> Dict[str, Any]:
        ...
        for result in self.results:
            ...
            if key not in endpoint_stats:
                endpoint_stats[key] = {"count": 0, "success_count": 0, "response_times": []}
        ...
        avg_target_met = avg_response_time <= (
            self.config.target_response_time_ms * 0.7
        )  # 70% of target for average
...
"""
        ...
        for endpoint, stats in analysis["endpoint_analysis"].items():
            status = (
                "✅" if stats["p95_response_ms"] <= self.config.target_response_time_ms else "⚠️"
            )
