...
class ProductionLoadTestRunner:
    ...
    def setup_test_scenarios(self):
        ...
        for scenario in self.scenarios:
            logger.info(
                f"   • {scenario.name}: {scenario.weight*100:.1f}% weight, {len(scenario.operations)} operations"
            )
    ...
    async def create_single_test_user(
        self, session: aiohttp.ClientSession, user_data: Dict[str, str]
        ...
        try:
            async with session.post(
                ...
                if response.status in [200, 201]:
                    ...
                    logger.debug(
                        f"Failed to create user {user_data['email']}: HTTP {response.status}"
                    )
                    return None
    ...
    async def execute_scenario_operation(
        ...
        try:
            ...
            if action == "signup":
                ...
                method = "PATCH"
                json_data = {"name": f"Updated {user_data['name']} {random.randint(1, 1000)}"}
    ...
    async def simulate_user_session(
        self, user_id: str, duration_minutes: float
    ) -> List[PerformanceMetric]:
        """Simulate a realistic user session for the specified duration"""
        session_metrics = []
        user_data = (
            random.choice(self.test_users).copy()
            if self.test_users
            else {
    ...
    async def execute_load_test_phase(
        self, phase_name: str, concurrent_users: int, duration_minutes: float
    ):
        """Execute a phase of the load test with specified parameters"""
        logger.info(
            f"🚀 Starting {phase_name}: {concurrent_users} users for {duration_minutes:.1f} minutes"
        )
    ...
    async def run_capacity_testing(self) -> List[PerformanceMetric]:
        ...
        for user_count in user_increments:
            ...
            metrics = await self.execute_load_test_phase(
                f"Capacity {user_count}", user_count, 2.0  # 2 minutes per capacity level
            )
            ...
            if metrics:
                ...
                if response_times:
                    ...
                    logger.info(
                        f"   📊 At {user_count} users: {avg_response_time:.1f}ms avg, {p95_response_time:.1f}ms p95"
                    )

                    # Stop capacity testing if performance degrades significantly
                    if (
                        p95_response_time > self.config.target_response_time_ms * 3
                    ):  # 3x target threshold
    ...
    def analyze_production_results(self) -> Dict[str, Any]:
        ...
        # SLA compliance
        sla_compliant_requests = len(
            [rt for rt in response_times if rt <= self.config.target_response_time_ms]
        )
        ...
        for scenario in self.scenarios:
            ...
            if scenario_metrics:
                scenario_response_times = [
                    m.response_time_ms for m in scenario_metrics if m.success
                ]
                if scenario_response_times:
                    scenario_stats[scenario.name] = {
                        "requests": len(scenario_metrics),
                        "success_rate": len([m for m in scenario_metrics if m.success])
        ...
        for endpoint, metrics in endpoint_stats.items():
            ...
            if successful:
                ...
                endpoint_analysis[endpoint] = {
                    ...
                    "cache_hit_rate": len([m for m in successful if m.cache_hit])
                    / len(successful)
                    * 100
                    if successful
                    else 0,
        ...
        return {
            ...
            "capacity_insights": {
                ...
                "recommended_capacity": self.config.max_concurrent_users
                if overall_sla_met
                else int(self.config.max_concurrent_users * 0.8),
            },
        }
...
"""
        ...
        for scenario_name, stats in analysis["scenario_analysis"].items():
            status = (
                "✅" if stats["p95_response_time"] <= self.config.target_response_time_ms else "⚠️"
            )
        ...
        for endpoint, stats in sorted(
            analysis["endpoint_analysis"].items(), key=lambda x: x[1]["p95_response_time"]
        ):
            status = (
                "✅" if stats["p95_response_time"] <= self.config.target_response_time_ms else "⚠️"
            )
            cache_info = (
                f", {stats['cache_hit_rate']:.1f}% cached" if stats["cache_hit_rate"] > 0 else ""
            )
...
async def main():
    ...
    parser.add_argument(
        "--sla-success", type=float, default=99.5, help="SLA success rate threshold"
    )
    parser.add_argument("--no-capacity", action="store_true", help="Skip capacity testing")
    parser.add_argument(
        "--no-multiregion", action="store_true", help="Skip multi-region simulation"
    )
