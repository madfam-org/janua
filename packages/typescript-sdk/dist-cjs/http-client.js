...
exports.createHttpClient = exports.AxiosHttpClient = exports.HttpClient = void 0;
const errors_1 = require("./errors");
const utils_1 = require("./utils");
/**
 * HTTP client with automatic token refresh and retry logic
 */
class HttpClient extends utils_1.EventEmitter {
    ...
    async request(config) {
        return this.executeWithRetry(async () => {
            ...
            if (response.status === 429) {
                ...
                throw new errors_1.RateLimitError('Rate limit exceeded', { rateLimitInfo });
            }
    ...
    async executeWithRetry(operation) {
        return utils_1.RetryUtils.withRetry(operation, this.config.retryAttempts, this.config.retryDelay, 2, // backoff factor
        30000 // max delay
        );
    }
    /**
     ...
     * Handle authentication errors
     */
    async handleAuthError(originalConfig) {
    ...
    async refreshTokens() {
        ...
        if (!refreshToken) {
            ...
            throw new errors_1.PlintoError('No refresh token available', 'AUTHENTICATION_ERROR');
        }
        try {
    ...
    async handleApiError(response) {
        ...
        throw errors_1.PlintoError.fromApiError(apiError);
    }
    /**
 ...
 */
class AxiosHttpClient extends utils_1.EventEmitter {
    constructor(config, tokenManager) {
        super();
        this.refreshPromise = null;
        this.tokenManager = tokenManager;
    ...
    setupAxiosInstance(config) {
        ...
        this.axios.interceptors.response.use((response) => response, async (error) => {
            ...
            if (error.response) {
                ...
                throw new errors_1.NetworkError('Request setup failed', error);
            }
        });
    }
 ...
 */
function createHttpClient(config, tokenManager) {
